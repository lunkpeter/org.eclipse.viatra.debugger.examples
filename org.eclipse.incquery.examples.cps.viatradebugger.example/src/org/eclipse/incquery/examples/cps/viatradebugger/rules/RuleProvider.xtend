package org.eclipse.incquery.examples.cps.viatradebugger.rules
...
public class RuleProvider {
	...
	public def getHostRule() {
		if (hostRule == null) {
			hostRule = createRule.name("HOSTRULE").precondition(HostInstanceMatcher.querySpecification).action(
				...
				cps2dep.remove(getCPSToDeployment_Traces, traceMatch.trace)
	...
	public def getApplicationRule() {
		if (applicationRule == null) {
			...
			applicationRule = createRule.name("APPRULE").precondition(ApplicationInstanceMatcher.querySpecification).action(
				CRUDActivationStateEnum.CREATED) [
	...
	public def getStateMachineRule() {
		if (stateMachineRule == null) {
			stateMachineRule = createRule.name("SMRULE").precondition(StateMachineMatcher.querySpecification).action(
				CRUDActivationStateEnum.CREATED) [
				...
				if (traces.empty) {
					...
					val trace = cps2dep.createChild(getCPSToDeployment_Traces, getCPS2DeplyomentTrace)
					trace.addTo(getCPS2DeplyomentTrace_CpsElements, stateMachine)
					trace.addTo(getCPS2DeplyomentTrace_DeploymentElements, behavior)

				} else {
					trace('''Adding new behavior to existing trace''')
					traces.head.addTo(getCPS2DeplyomentTrace_DeploymentElements, behavior)
				}
	...
	public def getTransitionRule() {
		if (transitionRule == null) {
			transitionRule = createRule.name("TRANSRULE").precondition(TransitionMatcher.querySpecification).action(
				CRUDActivationStateEnum.CREATED) [
				val depApp = engine.cps2depTrace.getAllValuesOfdepElement(null, null, appInstance).filter(
					DeploymentApplication).head
